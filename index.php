...
// Vars
$db = $argv[1];
$tables_to_skip = Array('poligonos');
$tables_to_import = Array('campanialances','campaniamareas');
		...
		*/
		...
		if($skip == false) { // Exept if we skip the table!
			...
			if($numrows == 0) {
				...
				$PgSchema = preg_replace("/COLLATE=[0-9a-zA-Z ()_]+/i", "", $PgSchema); // Remove the collate
				$PgSchema = preg_replace("/COLLATE [0-9a-zA-Z ()_]+/i", "", $PgSchema); // Remove the collate
				...
				//elimino los keys excepto el primario
				$PgSchema = preg_replace("/UNIQUE KEY [0-9a-z,() _]+/i", "", $PgSchema); // Remove the indexes except the primary key
